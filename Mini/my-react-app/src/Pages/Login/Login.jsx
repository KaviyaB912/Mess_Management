...
function Login() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (data.success) {
        // Pass the register number and aadhar number to student details page
        navigate("/student-details", { state: { register_number: registerNumber, aadhar_number: aadharNumber } });
      } else {
        setError(data.message || "Login failed");
      }
    } catch (error) {
